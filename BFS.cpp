...
{
	int startX = (int)startCell.x;
	int startY = (int)startCell.y;

	int endX = (int)endCell.x;
	int endY = (int)endCell.y;

	// first time setup
	if (!state.initialized)
	{
		state.previous = std::vector<std::vector<PathNode>>(gridWidth,
		...
		state.bfsQueue.push({ startX, startY });

		state.initialized = true;
	}

	if (state.bfsQueue.empty())
	{
	...
	PathNode current = state.bfsQueue.front();
	state.bfsQueue.pop();
